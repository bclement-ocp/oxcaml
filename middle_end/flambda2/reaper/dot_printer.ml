...
module Dual = struct
  ...
  module P = struct
    ...
    let all_names (t : Graph.graph) =
      let names = Hashtbl.create 100 in
      Code_id_or_name.Map.iter
        (fun name dep ->
          let dep_names =
            List.fold_right (fun dep acc -> dep_names dep @ acc) dep []
          in
          List.iter
    ...
    let edge ~ctx ppf src (dst : Graph.Dep.t) =
      let color, deps =
        match dst with
        | Alias { target } -> "black", [target]
        | Accessor { target; _ } -> "green", [target]
        | Constructor { target; _ } -> "blue", [target]
      in
      List.iter
        (fun dst ->
    ...
    let edges ~ctx ppf t =
      Code_id_or_name.Map.iter
        (fun src dst_set ->
          List.iter (fun dst -> edge ~ctx ppf src dst) dst_set)
        t
    ...
    let code_deps ~ctx ~code_id ~print_color ppf
      ...
      List.iter
        (fun v ->
          node ~ctx ~root:false ~print_color ppf (Code_id_or_name.var v))
        ((code_dep.exn :: code_dep.return) @ code_dep.params)
    ...
    let print ~ctx ~print_color ~print_name ppf
      ...
      Flambda_colours.without_colours ~f:(fun () ->
          Format.fprintf ppf "subgraph cluster_%d { label=\"%s\"@\n%a@\n%a}@."
            ctx print_name (nodes ~ctx ~print_color) t (edges ~ctx) t)
  end
  ...
  let print graph = print_dep (Code_id.Map.empty, graph)
end
*)
