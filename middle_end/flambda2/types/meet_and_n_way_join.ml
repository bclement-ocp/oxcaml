      ...
      (fun (immediates : _ Or_unknown.t) (id2, head2) ->
        ...
        | Ok head2 ->
          ...
          let immediates : _ Or_unknown.t =
            ...
            | Unknown, _ | _, Unknown -> Unknown
            | Known imms, Known imms2 ->
              Known
                (I.Map.update_many
                   (fun _ exts ext2 ->
                     match exts with
                     | None -> Some [id2, ext2]
                     | Some exts -> Some ((id2, ext2) :: exts))
                   imms imms2)
          in
          immediates)
      (Or_unknown.Known I.Map.empty) heads
  in
  let (immediates, env) : _ Or_unknown.t * _ =
    match immediates with
    | Unknown -> Unknown, env
    | Known immediates ->
      let join_env = ref env in
      let immediates =
        I.Map.map
          (fun exts ->
            (* Avoid starting the whole join machinery in the common case where
               all the extensions are empty anyways. *)
            if List.for_all (fun (_, ext) -> TEE.is_empty ext) exts
            then TEE.empty
            else
              let ext, env = n_way_join_env_extension !join_env exts in
              join_env := env;
              ext)
          immediates
      in
      Known immediates, !join_env
  in
  let head =
