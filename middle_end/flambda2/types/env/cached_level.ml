...
let with_database t ~database = { t with database }
...
let remove_unused_value_slots_and_shortcut_aliases
    ({ names_to_types; aliases; symbol_projections; database } as t)
    ~used_value_slots =
  ...
  let names_to_types =
    Name.Map.map_sharing
      ...
      names_to_types
  in
  (* The database is maintained in canonical form, no need to shortcut
     aliases. *)
  { names_to_types; aliases; symbol_projections; database }

let free_function_slots_and_value_slots
    { names_to_types; aliases = _; symbol_projections; database } =
  ...
  in
  (* TODO *)
  let _ = ignore database in
  let from_projections_and_database = from_projections in
  Name.Map.fold
