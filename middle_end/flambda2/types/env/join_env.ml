...
module Join_equations : sig
  ...
  let add_joined_simple ~joined_envs demoted_var canonicals { types } =
          ...
          let ty, known_uses =
            Simple.pattern_match canonical_simple
              ~const:(fun const ->
                ...
                ty, known_uses)
              ~name:(fun name ~coercion ->
                let ty = TG.apply_coercion (TE.find env name None) coercion in
                ty, known_uses)
          in
          let expanded =
  ...
  | Ok { demoted_in_target_env; demoted_in_some_envs; t = join_aliases } ->
    ...
    let rec loop equations_to_join joined_equations t =
      ...
      then
        ...
        let join_types, non_poison_uses =
          Name_in_target_env.Map.fold
            (fun name_in_target_env canonicals (join_types, known_uses) ->
              let canonicals =
                Simples_in_joined_envs.in_envs all_levels canonicals
              in
              let { Join_equations.t = join_types; non_poison_uses } =
              ...
              in
              let known_uses =
                add_known_use name_in_target_env non_poison_uses known_uses
              in
